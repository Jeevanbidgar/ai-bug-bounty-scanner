...
use ai_bug_bounty_scanner::tools::package_managers::{
    UpdateCheckerConfig, UpdateCheckerCoordinator, UpdateCheckerFactory,
};

#[tokio::test]
#[ignore = "manual smoke test: invokes locally available package managers and external registries"]
...
async fn test_update_checker_telemetry() {
    ...
    assert!(
        telemetry.update_checks_started >= 1,
        "Should start at least 1 update check"
    );
...
#[tokio::test]
#[ignore = "manual smoke test: available package managers depend on the host environment"]
async fn test_factory_creates_checkers() {
    ...
    assert!(!checkers.is_empty(), "Should create at least one checker");
