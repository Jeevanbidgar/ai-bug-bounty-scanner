use ai_bug_bounty_scanner::tools::package_managers::version_checker::{
    check_npm_update, check_gem_update, check_cargo_update,
};
...
async fn test_npm_update_check() {
    ...
    match result {
        Ok(check) => {
            ...
            assert!(check.current_version.is_some(), "Should have current version");
        }
        Err(e) => {
            println!("Error: {}", e);
...
async fn test_npm_package_not_installed() {
    ...
    match result {
        Ok(check) => {
            ...
            assert!(check.current_version.is_none(), "Should not have current version");
        }
        Err(_) => {
