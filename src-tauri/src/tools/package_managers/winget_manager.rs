...
impl WingetManager {
    ...
    pub async fn install(&self, winget_id: &str, tool_name: &str) -> Result<String> {
        ...
        self.emit_output(tool_name, &format!("Starting Winget installation for {}...\n", tool_name));

        // Get the WinGet executable path
        let winget_path = self.get_winget_path().await
            .ok_or_else(|| anyhow!("winget is not available. Please install App Installer from Microsoft Store."))?;

        eprintln!("📍 Using WinGet at: {}", winget_path);
        self.emit_output(tool_name, &format!("Using winget at: {}\n", winget_path));
        self.emit_output(tool_name, &format!("Installing {} via winget...\n", winget_id));
        ...
        let mut child = Command::new(&winget_path)
            .args(&["install", "--id", winget_id, "--accept-package-agreements", "--accept-source-agreements", "--silent"])
            .stdout(Stdio::piped())
            .stderr(Stdio::piped())
            .spawn()
        ...
        let status = child.wait().await.context("Failed to wait for winget install")?;

        if !status.success() {
            self.emit_output(tool_name, &format!("Failed to install {} via winget\n", tool_name));
            return Err(anyhow!("winget install failed"));
        }

        self.emit_output(tool_name, &format!("Successfully installed {} via winget\n", tool_name));
    ...
    pub async fn update(&self, winget_id: &str, tool_name: &str) -> Result<String> {
        ...
        let mut child = Command::new(&winget_path)
            .args(&["upgrade", "--id", winget_id, "--accept-package-agreements", "--accept-source-agreements", "--silent"])
            .stdout(Stdio::piped())
            .stderr(Stdio::piped())
            .spawn()
