        ...
        Err(_) => {
            ...
            if let Some(path) = find_executable_in_path("go").await {
                if let Ok((stdout, _stderr)) = execute_detection_command(&path, &["version"]).await {
                    ...
                    return PackageManagerInfo::available(PackageManagerType::Go, version, Some(path));
                }
            }
        }
    }
        ...
        Err(_) => {
            ...
            if let Some(path) = find_executable_in_path("pipx").await {
                if let Ok((stdout, _stderr)) = execute_detection_command(&path, &["--version"]).await {
                    ...
                    return PackageManagerInfo::available(PackageManagerType::Pipx, version, Some(path));
                }
            }
        }
    }
        ...
        Err(_) => {
            ...
            if let Some(path) = find_executable_in_path("cargo").await {
                if let Ok((stdout, _stderr)) = execute_detection_command(&path, &["--version"]).await {
                    ...
                    return PackageManagerInfo::available(PackageManagerType::Cargo, version, Some(path));
                }
            }
        }
    }
            ...
            Err(_) => {
                ...
                if let Some(path) = find_executable_in_path("apt").await {
                    if let Ok((stdout, _stderr)) = execute_detection_command(&path, &["--version"]).await {
                        ...
                        return PackageManagerInfo::available(PackageManagerType::Apt, version, Some(path));
                    }
                }
            }
        }
    ...
    #[cfg(target_os = "windows")]
    {
        ...
        for path in &search_locations {
            if let Ok((stdout, _stderr)) = execute_detection_command(path, &["--version"]).await {
                ...
                return PackageManagerInfo::available(PackageManagerType::WinGet, version, Some(path.clone()));
            }
        }
        ...
        if let Some(path) = find_executable_in_path("winget").await {
            if let Ok((stdout, _stderr)) = execute_detection_command(&path, &["--version"]).await {
                ...
                return PackageManagerInfo::available(PackageManagerType::WinGet, version, Some(path));
            }
        }
        ...
        if let Ok((stdout, _stderr)) = execute_detection_command(check_cmd, &check_args).await {
            if !stdout.trim().is_empty() {
                let version = parse_simple_version(&stdout);
                eprintln!("ℹ️  WinGet package detected but executable not found in common locations");
...
fn get_windows_executable_search_paths(exe_name: &str) -> Vec<String> {
    ...
    if let Ok(local_appdata) = std::env::var("LOCALAPPDATA") {
        ...
        paths.push(format!(r"{}\Microsoft\WindowsApps\{}", local_appdata, exe_name));
    ...
    if let Ok(user_profile) = std::env::var("USERPROFILE") {
        ...
        paths.push(format!(r"{}\scoop\apps\{}\current\{}", user_profile, exe_name.trim_end_matches(".exe"), exe_name));
