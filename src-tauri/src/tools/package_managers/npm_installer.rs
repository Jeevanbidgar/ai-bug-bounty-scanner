...
impl NpmInstaller {
    ...
    async fn install_nodejs(&self, tool_name: &str) -> Result<()> {
        ...
        #[cfg(target_os = "windows")]
        {
            ...
            if self.check_winget_available().await {
                self.emit_output(tool_name, "🔍 Found WinGet. Installing Node.js automatically...\n");
                ...
                match winget_manager.install("OpenJS.NodeJS", tool_name).await {
                    Ok(_) => {
                        self.emit_output(tool_name, "✅ Node.js installed successfully via WinGet\n");
            ...
            return Err(anyhow!("Node.js must be installed. See installation options above."));
        }
    ...
    pub async fn install(&self, tool: &ToolDefinition, tool_name: &str) -> Result<String> {
        self.emit_output(tool_name, &format!("🚀 Starting npm installation for {}...\n", tool.name));

        // Check if Node.js and npm are installed
        if !self.check_node_installed().await? || !self.check_npm_installed().await? {
            self.emit_output(tool_name, "⚠️  Node.js/npm not found. Attempting installation...\n");
            self.install_nodejs(tool_name).await?;
        }

        // Get the npm path using detection system
        let npm_path = self.get_npm_path().await
            .ok_or_else(|| anyhow!("npm is not available. Please install Node.js from: https://nodejs.org/"))?;

        self.emit_output(tool_name, &format!("📍 Using npm at: {}\n", npm_path));

        // Get the package name from npm_package field
        let package_name = tool.npm_package.as_ref()
            .ok_or_else(|| anyhow!("Tool {} does not have npm_package defined", tool.name))?;

        self.emit_output(tool_name, &format!("📦 Installing {} via npm globally...\n", package_name));
        ...
        let status = child.wait().await.context("Failed to wait for npm install")?;
        ...
        self.emit_output(tool_name, &format!("✅ Successfully installed {} via npm\n", tool.name));
    ...
    pub async fn verify_installation(&self, tool: &ToolDefinition) -> Result<String> {
        ...
        if output.status.success() {
            ...
            Err(anyhow!("{} is not properly installed or not in PATH", command_name))
        }
    }
    ...
    pub async fn update(&self, tool: &ToolDefinition, tool_name: &str) -> Result<String> {
        ...
        if !status.success() {
            ...
            return Err(anyhow!("npm update failed for {}. Check if the package is installed globally.", tool.name));
        }
