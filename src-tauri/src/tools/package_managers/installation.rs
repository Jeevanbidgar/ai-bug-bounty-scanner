...
pub async fn install_pipx() -> Result<InstallationResult, String> {
    ...
    match install_output {
        Ok(output) => {
            ...
            let ensurepath_output = execute_command_with_output(
                "pipx",
                &["ensurepath"],
                30,
            ).await;
            ...
            match ensurepath_output {
                ...
                Err(e) => {
                    steps.last_mut().unwrap().output = format!("⚠ PATH update failed: {}\nYou may need to restart your terminal manually.", e);
    ...
    if winget_check.is_ok() {
        ...
        match install_output {
            Ok(output) => {
                ...
                Ok(InstallationResult {
                    success: true,
                    message: "Go installed successfully! Restart your terminal to use go commands.".to_string(),
                    steps,
                    requires_restart: true,
                })
            },
...
pub async fn install_apt_package(package_name: &str) -> Result<InstallationResult, String> {
    ...
    let update_output = execute_command_with_output(
        "sudo",
        &["apt", "update"],
        60,
    ).await;
    ...
    match update_output {
        Ok(output) => {
            steps.last_mut().unwrap().success = true;
            steps.last_mut().unwrap().output = format!("✓ Package lists updated\n{}", 
            ...
            match install_output {
                Ok(output) => {
                    steps.last_mut().unwrap().success = true;
                    steps.last_mut().unwrap().output = format!("✓ {} installed successfully", package_name);
        ...
        Err(e) => {
            ...
            Ok(InstallationResult {
                success: false,
                message: format!("Failed to update package lists: {}. Check sudo permissions.", e),
                steps,
                requires_restart: false,
            })
        }
...
pub async fn install_winget_windows() -> Result<InstallationResult, String> {
    ...
    let open_result = execute_command_with_output(
        "cmd",
        &["/c", "start", "ms-windows-store://pdp/?ProductId=9NBLGGH4NNS1"],
        5,
    ).await;
    ...
    match open_result {
        Ok(_output) => {
            ...
            Ok(InstallationResult {
                success: true,
                message: "Microsoft Store opened. Please install 'App Installer' to get WinGet.".to_string(),
                steps,
                requires_restart: false,
            })
        },
        Err(e) => {
            steps.last_mut().unwrap().output = format!("✗ Failed to open Store: {}\n\nAlternative: Download from https://aka.ms/getwinget", e);
            Ok(InstallationResult {
                success: false,
                message: "Failed to open Store. Download WinGet from: https://aka.ms/getwinget".to_string(),
                steps,
                requires_restart: false,
            })
        }
